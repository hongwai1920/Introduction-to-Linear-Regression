...
class TwoLayerNet:
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        For a single datapoint x_i, the loss function of softmax is
        L_i = -\log\left(\frac{e^{f_{y_i}}}{ \sum_j e^{f_j} }\right)
        Differentiating with respect w_{y_i} (correct class) gives
        \nabla_{w_{y_i}} L_i = - x_i
        Differentiating with respect w_{j} where j \neq y_i (incorrect class) gives
        \nabla_{w_j} L_i = \frac{e^{w_j^Tx_i}}{\sum_j e^{w_j^Tx_i}}x_i = softmax \cdot x_i
        ...
        hidden_scores = np.maximum(fc1, 0)    # apply ReLu 
        scores = hidden_scores.dot(W2) + b2   # fully-connected layer
        ...
        scores = scores - np.max(scores, axis = 1, keepdims = True)  # avoid numerical instability
        ...
        # Calculate loss 
        loss = np.sum(-np.log(softmax_matrix[np.arange(N), y]))  # cross-entyopy loss
        loss /= N
        ...
        ##########################################################################
        ############ Compute the Backward pass: compute gradients ################
        ##########################################################################
        ...
        # W2 gradient
        dW2 = hidden_scores.T.dot(softmax_matrix)   # [HxN] * [NxC] = [HxC]
